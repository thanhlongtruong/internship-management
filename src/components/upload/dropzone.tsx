  ...
  (
    {
      ...
      className,
      disabled,
      dropMessageActive = "Tải lên file tại đây...",
      dropMessageDefault = "Kéo và thả file vào đây, hoặc nhấn để chọn",
      ...props
    },
    ref
  ) => {
      ...
      onDrop: (acceptedFiles, rejectedFiles) => {
        ...
        if (rejectedFiles.length > 0) {
          if (rejectedFiles[0]?.errors[0]) {
            ...
            const messages: Record<string, string> = {
              "file-too-large": `File quá lớn. Kích thước tối đa là ${formatFileSize(
                maxSize ?? 0
              )}.`,
              "file-invalid-type": "Không hỗ trợ loại file này.",
              "too-many-files": `Bạn chỉ có thể tải lên tối đa ${
                maxFiles ?? "files"
              } file(s).`,
              default: "Không hỗ trợ loại file này.",
            };
        ...
        if (maxFiles) {
          ...
          if (acceptedFiles.length > remainingSlots) {
            setError(`Bạn chỉ có thể tải lên tối đa ${maxFiles} file(s).`);
            return;
          }
        }
          ...
          <div className="flex flex-col items-center justify-center gap-2 text-center text-muted-foreground">
            ...
            {(!!maxSize || !!maxFiles) && (
              <div className="text-xs">
                {maxFiles && maxFiles > 1 ? `Tối đa ${maxFiles} file` : ""}
                {maxFiles && maxFiles > 1 && maxSize ? ", " : ""}
                {maxSize && `Kích thước tối đa: ${formatFileSize(maxSize)}`}
              </div>
            )}
          </div>
        </div>
